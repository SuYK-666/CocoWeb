...
_PROMPT_LEAK_LINE_PATTERNS = [
    re.compile(r"^(好的[，,]?\s*)?作为一名.*智能体.*$"),
    re.compile(r"^我将基于.*(撰写|生成).*(报告|分析).*$"),
    re.compile(r"^(以下|下面)是.*(报告|分析).*$"),
    re.compile(r"^输出要求[:：]?$"),
    re.compile(r"^硬性要求[:：]?$"),
    re.compile(r"^仓库事实[:：]?$"),
    re.compile(r"^项目技术名片[:：]?$"),
    re.compile(r"^请基于仓库事实与项目技术名片.*$"),
]
...
def _remove_prompt_leak_lines(markdown_text: str) -> str:
    ...
    for line in markdown_text.splitlines():
        stripped = line.strip()
        if any(pattern.search(stripped) for pattern in _PROMPT_LEAK_LINE_PATTERNS):
            continue
